...
function patchChildren(n1, n2, container) {
      ...
      // 如果 n2 的 children 是text 类型的话
      // 就看看和之前的 n1 的 children 是不是一样的
      // 如果不一样的话直接设置一下 text 即可
...
function setupStatefulComponent(instance) {
      ...
      const setupContext = instance.setup && instance.setup(instance.props)
      ...
      instance.update = effect(
            function componentEffect() {
               if (!instance.isMounted) {
                  ...
                  // 是因为在effect 内调用 render 才能触发依赖收集
                  ...
                  // 这里几月 subTree 再次调用 patch
                  // 基于 render 返回的 vnode，再次进行渲染
                  // 这里我们把这个行为隐喻成开箱
                  // 一个组件就是一个箱子
                  // 里面有可能是 element（也就是可以直接渲染）
                  // 也有可能还是 component
                  // 这里就是递归的开箱
